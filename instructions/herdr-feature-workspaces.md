...
- Create the workspace with `herdr worktree create --cwd "$repo_root" --branch "wheels/$slug" --base "$base_ref" --path "$repo_root/.worktrees/$slug" --label "$slug" --no-focus --json`. Always pass the primary repository root through `--cwd`; relying on the process working directory can make Herdr treat a linked worktree as the source.
